...
Local Existing Instances monoid_assoc monoid_comm
...
(** * Big ops over lists *)
Fixpoint big_opL {M : Type} {o : M → M → M} `{!Monoid o} {A} (f : nat → A → M) (xs : list A) : M :=
  match xs with
  | [] => monoid_unit
...
Local Definition big_opM_def {M : Type} {o : M → M → M} `{!Monoid o} `{Countable K} {A} (f : K → A → M)
...
Local Definition big_opS_def {M : Type} {o : M → M → M} `{!Monoid o} `{Countable A} (f : A → M)
...
Local Definition big_opMS_def {M : Type} {o : M → M → M} `{!Monoid o} `{Countable A} (f : A → M)
...
Section big_op.
Context {M : Type} {o : M → M → M} `{!Monoid o}.
...
Section homomorphisms.
  Context {M1 M2 : Type} {o1 : M1 → M1 → M1} {o2 : M2 → M2 → M2} `{!Monoid o1, !Monoid o2}.
  ...
  Proof using Type*. intros. by apply big_opS_commute1. Qed.
  ...
  Proof using Type*. intros. by apply big_opMS_commute1. Qed.
End homomorphisms.
