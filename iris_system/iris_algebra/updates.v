...
(* This quantifies over [option A] for the frame.  That is necessary to
   make the following hold:
     x ~~> P → Some c ~~> Some P
*)
Definition cmra_updateP {A : cmra} (x : A) (P : A → Prop) := ∀ mz,
  ✓ (x ⋅? mz) → ∃ y, P y ∧ ✓ (y ⋅? mz).
Global Instance: Params (@cmra_updateP) 1 := {}.
Infix "~~>:" := cmra_updateP (at level 70) : iris_algebra_scope.

Definition cmra_update {A : cmra} (x y : A) := ∀ mz,
  ✓ (x ⋅? mz) → ✓ (y ⋅? mz).
Infix "~~>" := cmra_update (at level 70) : iris_algebra_scope.
...
Proof.
  rewrite /pointwise_relation /cmra_updateP=> x x' Hx P P' HP;
    split=> ? mz; setoid_subst; naive_solver.
Qed.
...
Lemma cmra_update_exclusive `{!Exclusive x} y:
  ✓ y → x ~~> y.
Proof. move=>?[z|]=>[/exclusive_l[]|_]. done. Qed.
...
Proof. intros mz. rewrite comm cmra_op_opM_assoc. apply cmra_valid_op_r. Qed.
...
Lemma cmra_update_valid x y : (✓ x → x ~~> y) → x ~~> y.
Proof.
  intros H mz Hmz. apply H, Hmz.
  destruct mz.
  - eapply cmra_valid_op_l, Hmz.
  - apply Hmz.
Qed.
...
Proof.
  ...
  destruct (Hup (core x)) as (y&?&?); first by rewrite cmra_core_r.
...
Proof.
  unfold cmra_updateP. done.
Qed.
Lemma cmra_discrete_update (x y : A) :
  x ~~> y ↔ ∀ mz, ✓ (x ⋅? mz) → ✓ (y ⋅? mz).
Proof.
  unfold cmra_update. done.
Qed.

Lemma cmra_discrete_total_updateP `{CmraTotal A}
    (x : A) (P : A → Prop) :
  x ~~>: P ↔ ∀ z, ✓ (x ⋅ z) → ∃ y, P y ∧ ✓ (y ⋅ z).
Proof.
  rewrite cmra_total_updateP. done.
Qed.
Lemma cmra_discrete_total_update `{CmraTotal A} (x y : A) :
  x ~~> y ↔ ∀ z, ✓ (x ⋅ z) → ✓ (y ⋅ z).
Proof.
  rewrite cmra_total_update. done.
Qed.
End updates.
...
Section option.
  ...
  Proof.
    intros Hx Hy; apply cmra_total_updateP. intros [y|] ?.
